...
models.0 <- with(d2011, summary(lmer(BSMMAT ~ 1+(1|IDCLASS))))
(sm0 <- MIsummary(models.0))

mbb0 <- with(d2011, fun = function(x) MYboot(x, formula(BSMMAT ~ 1+(1|IDCLASS)), R = 1000))
(smb <- MIsummary(mbb0))

mbb01 <- with(d2011, fun = function(x) MYboot(x, formula(BSMMAT ~ 1+(1|IDCLASS)), nsmpl = 1000, R = 1000))
(smb01 <- MIsummary(mbb01))

mbb02 <- with(d2011, fun = function(x) 
  MYboot(x, formula(BSMMAT ~ 1+(1|IDCLASS)), strata = data.2011$IDCLASS, R = 1000))
...
<<echo = F>>=
tb <- cbind(c(sm0$coefficients[,1], sm0$varcor[[1]][1], sm0$sigma),
            c(smb$coefficients[,1], smb$varcor[[1]][1], smb$sigma),
            c(smb01$coefficients[,1], smb01$varcor[[1]][1], smb01$sigma),
            c(smb02$coefficients[,1], smb02$varcor[[1]][1], smb02$sigma))
rownames(tb) <- c("Intercept", "tauBeta00", "sigma")
colnames(tb) <- c("REML", "nnREML", "mnREML", "nnIDclassREMSL")
xtable(tb)
@
\end{document}
